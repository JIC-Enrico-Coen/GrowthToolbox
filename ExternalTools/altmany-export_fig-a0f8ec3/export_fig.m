...
%
% 20160719  Edited by RK to eliminate any requirement that the handles to
% figure elements be of any particular type.  Changes also made to
% isolate_axes.m.
%
% Examples:
...
%}
    ...
    % RK: wrong. Instead, handle the case where fig is a figure, and
    % otherwise accept whetever array of handles fig is.
    ...
    if hasTransparency ...
            && options.wantfile  % RK 2016 Apr 11: No format warning needed when no file requested.
    ...
    try
        ...
        if isbitmap(options)
            ...
            if options.transparent && (options.png || options.alpha)
                ...
                allpositions = cell(1,length(allchildren));
                for i=1:length(allchildren)
                    try
                        allpositions{i} = get(allchildren(i),'Position'); 
                    catch
                        allpositions{i} = [];
                    end
                end
                ...
                tcol = get(fig, 'Color');
                % Set background colour to white, and set size in case it was
                % changed internally
%         texts = findall(fig,'Type','text');
%         textpos = get(texts,'Position');
%         textpos = cell2mat(textpos)
                DISCARD = print2array(fig, 0.1, renderer);
%                 imwrite(DISCARD,'D.png');
                pause(0.05);
%         texts = findall(fig,'Type','text');
%         textpos = get(texts,'Position');
%         textpos = cell2mat(textpos)
                ...
                A = captureImage( 'w' );
%                 imwrite(uint8(A),'A.png');

                B = captureImage( 'k' );
%                 imwrite(uint8(B),'B.png');
...
function img = captureImage( color )
    % Set the background colour, and set size in case it was
    % changed internally
    set(fig, 'Color', color, 'Position', pos);
    for i=1:length(allchildren)
        if ~isempty( allpositions{i} )
            set( allchildren(i), 'Position', allpositions{i} );
        end
    end
    ...
    try
        % Print large version to array
        img = print2array(fig, magnify, renderer);
        % Downscale the image
        img = downsize(single(img), options.aa_factor);
    catch
        % This is more conservative in memory, but kills transparency (issue #58)
        img = single(print2array(fig, magnify/options.aa_factor, renderer));
    end
end
end
...
function [fig, options] = parse_args(nout, fig, varargin)
    ...
    % Set the defaults
    options = struct(...
        'name', '', ...  % RK 2016 Apr 11: By default there is no output file.
        'crop', true, ...
    ...
    for a = 1:nargin-2
        ...
        % Addition by Richard Kennaway 2016 Apr 11.
        % This is used to suppress format warnings when no output file has
        % been requested.
        options.wantfile = options.pdf || ...
                           options.eps || ...
                           options.png || ...
                           options.tif || ...
                           options.jpg || ...
                           options.bmp;
        if options.wantfile && isempty( options.name )
