function [pout, pbaryout, vout, viout, facevout, faceiout, fviout, feiout] = select3d(obj)
...
isline = false;
% isperspective = false;
% Above line commented out by RK, since perpective projection is not
% handled.
...
if nargout>1
    ...
    min_dist = min(dist);
    min_index = find(dist==min_dist,1); % Amended by RK to ensure exactly one vertex is found.
    
    % Get closest vertex index and vertex
    viout = faces(faceiout,min_index);
    vout = vert(:,viout);
    
    pbaryout = baryCoords( [v1'; v2'; v3'], [], pout' );
    [ignore,fviout] = max( pbaryout );
    [ignore,feiout] = min( pbaryout );
end
