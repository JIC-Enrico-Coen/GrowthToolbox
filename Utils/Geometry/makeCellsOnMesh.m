function celllayer = makeCellsOnMesh( vxs, tris, tnormals, d )
%cells = makeCellsOnMesh( vxs, tris, normals, d )
%   A method of creating a layer of cells over a mesh of any shape, with or
%   without a border.
...
%         tisi = tis{i};
        ...
        pts(1,:) = vpts(1,:);
        fes(1) = tsi( 1 );
        bcs(1,find(tris(fes(1),:)==vsi(1),1)) = 1;
        while j < numpts
            totd = delta*j;
            while dists(k) < totd
                k = k+1;
    ...
    if isempty(ptloops)
        ...
        fe = 1;
        bc = [1 1 1]/3;
        pt = baryToGlobalCoords( fe, bc, vxs, tris );
        ...
        bcs = zeros(numpts,3);
        for i=1:numpts
            [pts(i,:),fes(i),bcs(i,:)] = projectToMesh( pt + offsets(i,:), vxs, tris, tnormals, fe );
        end
        numallpts = 7;
    ...
    minsize = avsz*1;
    maxsize = avsz*1;  % Er, wut??  minsize==maxsize makes no sense.
    

    % Calculate the exterior angle between each boundary edge and the next.
    normals = tnormals(fes,:);
...
%         allanglescheck = surfaceangle( deltas([end 1:(end-1)],:), deltas, fenormals );
...
%         end
        ...
        for i=1:numnewpts
            ...
            [newpts(i,:),newfes(i),newbcs(i,:)] = projectToMesh( b + newpt, vxs, tris, tnormals, fes([ai bi ci]) );
        end
        ...
        if ~borderjoin
            % Calculate the new fenormals, deltas, angles, indexes, and triangles.
            newnormals = tnormals(newfes,:);
            % allpts = [ allpts; newpts ];
...
%         end
    ...
    bcs = zeros( numvxs, 3 );
    for i=1:numvxs
        [ fes(i), bcs(i,:), bcerr, abserr ] = findTriangle( vxs, tris, tnormals, celllayer.cell3dcoords(i,:) );
    end
    ...
    celllayer.cellcolor = [];
    celllayer.cellvalues = zeros(numcells,0);
    celllayer.valuedict = struct( 'name2IndexMap', struct(), 'index2NameMap', [], 'case', -1 );
    celllayer.valuedict.index2NameMap = {};
    celllayer.cell3dcoords = baryToGlobalCoords( celllayer.vxFEMcell, celllayer.vxBaryCoords, vxs, tris );
end
...
function [q,ci,bc] = projectToMesh( p, vxs, tris, trinormals, hint )
    [ ci, bc, bcerr, abserr ] = findTriangle( vxs, tris, trinormals, p, 'hint', hint );
    q = baryToGlobalCoords( ci, bc, vxs, tris );
end
