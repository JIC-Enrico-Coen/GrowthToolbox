function [cells,cellvxs,centres] = makeVoronoiSemiEllipse( numcells, numiters, bbox, subdivideedge )
%[cells,cellvxs,centres] = makeVoronoiSemiEllipse( numcells, numiters, bbox, subdivideedge )
...
%   yhi].
    ...
    vcentre = [ (vmin(1)+vmax(1))/2, vmin(2) ]; % (vmin + vmax)/2;
    semidiam = [ (vmax(1)-vmin(1))/2, vmax(2)-vmin(2) ]; % (vmax - vmin)/2;
    a = semidiam(1);
    b = semidiam(2);
    c = a+a+b;
    e = b*1.2;
    ...
    [cellvxs,cells] = truncateVoronoiToSemiEllipse( bbox, cellvxs, cells, subdivideedge );
    if isempty(cellvxs)
...
function [vvxs,vcells] = truncateVoronoiToSemiEllipse( bbox, vvxs, vcells, subdivideedge )
%[vvxs,vcells] = truncateVoronoiToSemiEllipse( bbox, vvxs, vcells )
...
%   The point at infinity is assumed to be the first vertex in vvxs.
    ...
    semiaxes = [ (bbox(2)-bbox(1))/2, bbox(4)-bbox(3) ]; % (bbox([2 4]) - bbox([1 3]))/2;
    centre = [ (bbox(2)+bbox(1))/2, bbox(3) ]; % (bbox([2 4]) + bbox([1 3]))/2;
    semiEllipseArea =  semiaxes(1)*semiaxes(2)*pi/2;
    areaPerCell = semiEllipseArea/size(vvxs,1);
    typicalCellDiam = 2 * sqrt(areaPerCell/pi);
    pointSpacing = typicalCellDiam/2;
    maxSegmentAngle = min( pointSpacing/min(semiaxes), pi/8 );
    numEllipsePoints = round( pi/maxSegmentAngle );
    angle = linspace( 0, pi, numEllipsePoints+1 )';
    ca = cos(angle);
    sa = sin(angle);
    polyvxs = [ca,sa].*semiaxes + centre;
    polyedges = [ (1:length(angle))', [2:length(angle), 1]' ];
