function  m=leaf_vertex_follow_mgen(m,varargin) %realtime,RegionLabels,Morphogens,start_figno)
...
%         disp(sprintf('m.monitor_figs=%d ',m.monitor_figs))
    ...
    %if isempty( vertex_set ), return; end % RK 2010-03-18
    colours='crgbmk';
    for i=1:N
...
%             plot(y(:)/max(y(:)),z(:),'-*','color',colours(1+rem(i,length(colours))));
            ...
            % Records the concentration of a speficied morphogen at
            % specified steps. 
            % ERIKA 
             ...
             if mod(rounddec(m.globalDynamicProps.currenttime, 3),(Stime*interv))==0                
                ...
                %Erika   % save for the right morphogen 
                m.userdata.gradients.name2index{i} = Morphogens{i};
                %m.userdata.gradients.petiole{i} = y(size(m.nodes(m.userdata.petlength,2),1));
                ...
                m.userdata.gradients.POSACT{pos+1,i} = y(:);
                    % time of recording
                m.userdata.gradients.TIME{pos+1,i} = rounddec(m.globalDynamicProps.currenttime,2);
                    % find the base node of the FE mesh
%                 startN = min(m.nodes(:,2));              
%                 m.userdata.ERIKA(pos+1).StartN = startN;
%                     % track one node a given distance from the base away
%                 m.userdata.ERIKA(pos+1).OBPOS = m.nodes(m.userdata.trackedNode,2);
%                     % the concentration at this tracked node
%                 m.userdata.ERIKA(pos+1).OBCONC= m.morphogens(m.userdata.trackedNode,getMgenLevels(m, 'POLARISER'));
              end
            end
            ...
            if rounddec(m.globalDynamicProps.currenttime,3) == rounddec(endtime,3); 
               ...
               '_',namestr,'_DifPattern.mat'], 'data');
               
%                gr = num2str(m.userdata.GROWTH); dil = num2str(m.userdata.DILUTION);
%                save([path,filesep,'TimeSeries\Data',filesep,d,'_end',en,'_interval',inte,...
%                     '_growth', gr,'_dilution',dil,'_DiffusionPattern.mat'], 'eri');
                % also try to get an overview figure;
            %  plotConcSummary_ActualData(data);  % Commented out by RK as
            %  it doesn't exist.
            end
        ...
        theaxes = figureaxes(start_figno);
            ...
            xlabel(theaxes(end),['distance along seamed line (model: ',m.globalProps.modelname,')']);
            ylabel(theaxes(end),'Actual Concentration');
