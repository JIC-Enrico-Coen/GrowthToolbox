...
%   and used to calculate the length of its first step.
    ...
    [m,newbranchinfo] = spawnBranches( m, branchingTubules, branchSegments, branchSegbc, branchTimes, branchSides, branchAngles );
    
    % Update stats.
%     if ~isfield(  m.tubules.statistics, 'spontbranchings' )
%         m.tubules.statistics.spontbranchings = 0;
%     end
%     m.tubules.statistics.spontbranchings = m.tubules.statistics.spontbranchings + numNewTubules;
