function m = gftboxtests(f)
    ...
    outputDir = fullfile( toolboxDir, 'test', 'tortureresult' );
    ...
    [ok,msg,msgid] = mkdir( [ GFtboxDir(), '/test' ] );
    if ~ok
        warning( msg, msgid );
        return;
    end
    ...
    m = testcmd( m, 'leaf_block3D', 'size', [4 3 2], 'divisions', [4 3 2] );
    m = leaf_plotoptions( m, 'drawsecondlayer', true, 'bioAemptyalpha', 0 );
    m = leaf_makesecondlayer( m, 'mode', 'box', 'divisions', [7 5 3], 'subdivisions', [2 2 2] );
    m = show( m );
    m = leaf_makesecondlayer( m, 'mode', 'rectgrid', 'divisions', [7 5], 'subdivisions', [2 2] );
    m = show( m );
    
    m = testcmd( m, 'leaf_cylinder', 'xwidth', 2, 'ywidth', 2, 'height', 2, 'circumdivs', 8, 'heightdivs', 2 );
    seamnodes = abs(m.nodes(:,1)) < 0.01;
    m = show( m );
    m = testcmd( m, 'leaf_snapshot', [ GFtboxDir(), '/test/tortureresult/torturetestpic1.png' ], 'newfile', true );
    m = testcmd( m, 'leaf_snapshot', [ GFtboxDir(), '/test/tortureresult/torturetestpic2.png' ], 'newfile', true, 'hires', true );
    ...
    m = show( m );
    m = testcmd( [], 'leaf_circle', 'xwidth', 2, 'rings', 4 );
    m = show( m );
    ...
    m = show( m );
    m = testcmd( m, 'leaf_makesecondlayer', 'mode', 'few', 'numcells', 30, 'absdiam', 0.1 );
    m = show( m );
    fprintf( 1, 'Showing second layer, mode ''few''.\n' );
    
    m = testcmd( m, 'leaf_add_cellfactor', 'c_a', 'c_b', 'c_c' );
    m = testcmd( m, 'leaf_delete_cellfactor', 'c_d', 'c_b' );
    m = testcmd( m, 'leaf_add_cellfactor', 'c_a', 'c_d', 'c_b' );
    m = testcmd( m, 'leaf_rename_cellfactor', 'c_a', 'c_e' );
    m = testcmd( m, 'leaf_set_cellfactor', 'factor', 'c_e', 'operation', 'radial', 'amount', 1 );
    m = testcmd( m, 'leaf_set_cellfactor', 'factor', 'c_b', 'operation', 'linear', 'amount', [-1 1] );
    m = testcmd( m, 'leaf_set_cellfactor', 'factor', 'c_c', 'operation', 'random', 'amount', 1, 'allowedvalues', 'bool' );
    m = testcmd( m, 'leaf_plotpriority', {'c_e', 'c_b', 'c_c'}, [1 1 2], 0.5, 'type', 'cellvalue' );
    m = testcmd( m, 'leaf_plotoptions', 'cellbodyvalue', 'c_e' );
    m = show( m );
    m = testcmd( m, 'leaf_plotoptions', 'cellbodyvalue', {'c_e','c_b','c_c'} );
    m = testcmd( m, 'leaf_setcellcolorinfo', 'factor', 'c_e', 'mode', 'posneg', 'pos', [1 0 0], 'neg', [0 1 1] );
    m = testcmd( m, 'leaf_setcellcolorinfo', 'factor', 'c_b', 'mode', 'posneg', 'pos', [1 1 0], 'neg', [0 0 1] );
    m = testcmd( m, 'leaf_setcellcolorinfo', 'factor', 'c_c', 'mode', 'posneg', 'pos', [0 1 0], 'neg', [1 0 1] );
    m = testcmd( m, 'leaf_setcellcolorinfo', 'factor', 'c_d', 'mode', 'posneg', 'pos', [0 0 1], 'neg', [1 1 0] );
    m = show( m );
...
function m = testcmd( m, cmd, varargin )
    ...
    m = f( m, varargin{:} );
    CMDSTRUCT.(cmd) = true;
    show_ok = validmesh(m,true,false);
    if ~show_ok
        xxxx = 1;
    end
end
...
function m = show( m, mgen )
    ...
    if dofigures
        m = leaf_save( m, sprintf( 'testfig%04d.fig', framecount ), outputDir, 'overwrite', 1 );
    end
